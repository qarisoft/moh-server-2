...
import React, {FC, FormEvent, PropsWithChildren, useCallback, useMemo, useState} from 'react'
import PageLayout from "@/Pages/Projects/layout";
import {Minus, Plus, Trash, Trash2, TrashIcon} from "lucide-react";
        ...
        <Dialog onOpenChange={onOpen} open={open} modal>
            ...
            <DialogDescription>انشاء مشروع جديد</DialogDescription>
                ...
                <div className="space-y-1 p-4">
                    ...
                    <Input onChange={(e)=>setData('name',e.target.value)} id="name" error={errors.name} label={'الاسم'}/>
                    <div className="flex justify-between pt-2">
                        ...
                        <div className="flex gap-2 pb-2 items-center">
                            ...
                            <Btn rounded onClick={()=>setData(c=>({...c,items:[...c.items,{floor:1,apartments:1,id:c.items.length}]}))}>
                                ...
                                {data.items.map((item,i)=>(
                                    <tr key={i} className={'group'}>
                                        ...
                                        <td className={'bg-gray-00  h-fit'}>
                                                <Btn a={''} b={''}
                                                     onClick={()=>setData(c=>({...c,items:c.items.filter((it)=>it.id!=item.id)}))}
                                                    children={
                    ...
                    <Textarea onChange={(e)=>setData('description',e.target.value)} id="description" label={'الوصف'} error={errors.description} />
                </div>
